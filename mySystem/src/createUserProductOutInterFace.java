...
public class createUserProductOutInterFace extends JPanel{
	private final static long serialVersionUID = 230000123132212L;
	...
	public createUserProductOutInterFace() {
		// TODO 自动生成构造函数存根
	...
	private void Input(){
		...
		String number = product_value.getText().trim();
		...
		String baba[] = {id,client,name,spec,unit,value,number,make,date,text}; 
		...
		if(!DBUtil.isExist("select * from product_in where product_ID="+"'"+baba[0]+"'")){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意编号不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_id.setText("");
			product_id.requestFocus();
			return;
		}
		if(!DBUtil.isExist("select * from product_in where product_client="+"'"+baba[1]+"'")){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意客户名不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_client.setText("");
			product_client.requestFocus();
			return;
		}	
		if(!DBUtil.isExist("select * from product_in where product_name="+"'"+baba[2]+"'")){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意产品名不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_name.setText("");
			product_name.requestFocus();
			return;
		}	
		if(!DBUtil.isExist("select * from product_in where product_spec="+"'"+baba[3]+"'")){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意规格不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_spec.setText("");
			product_spec.requestFocus();
			return;
		}	
		if(!DBUtil.isExist("select * from product_in where product_unit="+"'"+baba[4]+"'")){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意单位不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_unit.setText("");
			product_unit.requestFocus();
			return;
		}	
		if(!DBUtil.isExist("select product_value from product_in where product_value >="+num)){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意要发货的数量超出了库存数量，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_value.setText("");
			product_value.requestFocus();
			return;
		}
		if(!DBUtil.isExist("select * from product_in where product_make="+"'"+baba[6]+"'")){
			javax.swing.JOptionPane.showMessageDialog(null, "请注意产品生产部门不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
			product_make.setText("");
			product_make.requestFocus();
			return;
		}	
		//if(!DBUtil.isExist("select * from product_in where stock_date="+"'"+da+"'")){
		//	javax.swing.JOptionPane.showMessageDialog(null, "请注意入库日期不存在，不能录入，请更改之后重新输入！","数据有误",javax.swing.JOptionPane.ERROR_MESSAGE);
		//	stock_date.setText("");
		//	stock_date.requestFocus();
		//	return;
		//}	
			sql = "'"+id +"','"+client+"','"+name+"','"+spec+"','"+unit+"',"+value+",'"+make+"','"+date+"','"+text+"'";
			sql = "insert into product_out values("+sql+")";
			USeDB.UpdateDB(sql);
