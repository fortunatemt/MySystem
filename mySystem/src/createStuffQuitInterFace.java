...
private void IsGood(){
	...
	sql = "select stuff_ID,stuff_name,stuff_company,stuff_unit from stuff_in where stuff_ID="+id+" and stuff_name="+name+" and stuff_company="+company+" and stuff_unit="+unit;
	if(!DBUtil.isExist(sql)){
		javax.swing.JOptionPane.showMessageDialog(null,"请认真核对你的输入项是否和以前的相等，请务必输入相同的编号、名称、公司、规格！","重新输入",javax.swing.JOptionPane.ERROR_MESSAGE);
		clean();
		return;
	}
	sql ="delete from stuff_in where stuff_ID="+id+" and stuff_name="+name+" and stuff_company="+company+" and stuff_unit="+unit;
	USeDB.UpdateDB(sql);
	sql = "'"+id+"','"+name+"','"+company+"','"+people+"','"+check+"','"+unit+"','"+value+"','"+spec+"','"+color+"','"+place+"','"+date+"','"+text+"'";
	sql = "insert into stuff_quit values("+sql+")";
	USeDB.UpdateDB(sql);
	clean();
